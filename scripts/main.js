...
function updateCirclePosition(event){
    if(firstClick){
        ...
        newX = Math.max(gameArea.left + circleRadius, Math.min(gameArea.right - circleRadius, newX));
        newY = Math.max(gameArea.top + circleRadius, Math.min(gameArea.bottom - circleRadius, newY));
        circleX = newX;
        circleY = newY;
    }
...
function checkCollisions() {
    ...
    for (let i = 0; i < planets.length; i++) {
        ...
        if (distance < planets[i].radius + circleRadius + 30) {
            ...
            if (i == 2){
                circleCenterX = planets[i].x + planets[i].radius * Math.cos(angle)*1.35;
                circleCenterY = planets[i].y +  planets[i].radius * Math.sin(angle)*1.35;
            } else if (i == 1){
                circleCenterX = planets[i].x + planets[i].radius * Math.cos(angle)*1.5;
                circleCenterY = planets[i].y +  planets[i].radius * Math.sin(angle)*1.5;

            } else{
                circleCenterX = planets[i].x + planets[i].radius * Math.cos(angle)*1.55;
                circleCenterY = planets[i].y +  planets[i].radius * Math.sin(angle)*1.55;
            }
